...
class MSF:
    ...
    def omsc(self, long_frame_power_spectrum: list, mod_fft_size: int):
        ...
        for bin_num in long_fft_bin_sum:
            ...
            # Modulation spectrum
            mod_spectrum = FFT.power_fft(zero_padded_bin_sum, mod_fft_size) + np.finfo(float).eps
            ...
            valley_array.append(min(np.log10(mod_spectrum[1:round(self.binstep / 2)] + np.finfo(float).eps)) + np.finfo(float).eps)
